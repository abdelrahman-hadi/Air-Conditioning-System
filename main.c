...
void msDelay(unsigned int value){ /*Provides delay in Miliseconds equal to the argument provided.Note that the choice of loop parameter 1275 is completely determined by the inernal design of Compiler and may vary from Compiler to Compiler.*/
unsigned int x,y;
for(x=0;x<value;x++)
...
unsigned char x1[5]="FANON"; /*TEMPR: means Temperature.*/
unsigned char x2[5]="HTRON"; /*HTRON means Heater on.*/
...
while(1){  /*Infinite loop is made because of the absence of Operating System. Because there is no operating system to return to*/
